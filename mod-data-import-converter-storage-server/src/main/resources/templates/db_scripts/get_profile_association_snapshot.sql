...
CREATE OR REPLACE FUNCTION
  ...
  RETURNS TABLE(association_id uuid, master_id uuid, master json, master_type text, detail_id uuid, detail_type text, detail_order integer, detail json, react_to text)
    AS $$
      BEGIN
        RETURN query
          EXECUTE format('
